...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            await self.browser_context.add_init_script(path=os.path.join(current_dir, '..', '..', 'libs', 'stealth.min.js'))
...
# v2: modify to enable small (<20) number of CRAWLER_MAX_NOTES_COUNT
    ...
    async def search(self) -> None:
        ...
        note_num = None
        if config.CRAWLER_MAX_NOTES_COUNT < xhs_limit_count:
            # If the number of notes to be crawled is less than minimum notes in a xhs page
            note_num = config.CRAWLER_MAX_NOTES_COUNT
            config.CRAWLER_MAX_NOTES_COUNT = xhs_limit_count
        start_page = config.START_PAGE
        # v2: modify to allow only one keyword
        # for keyword in config.KEYWORDS.split(","): # 只能有一个keyword
        for keyword in [config.KEYWORDS]:
            ...
            while (
                ...
                try:
                    ...
                    if note_num:
                        # If only a few notes are needed, crop the result.
                        notes_res["items"] = notes_res["items"][:note_num]
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.search] Search notes res:{notes_res}"
                    )
                    if not notes_res or not notes_res.get("has_more", False):
